...
% if(status)
    numPyd = 3;
    [ mvCurLevel, mvPyd  ] = pyramidal_sgm( I0, I1, numPyd );
% else
...
if(status) 
    flow = convert_index_to_vector(D1, 3);
    flow(:,:,3) = ~isnan(D1);
    flow2 = flow;
else
    ...
    D1Image = disp_to_color(D1, 64);
    filterD1(isnan(filterD1)) = 0;
    filterD1Image = disp_to_color(filterD1, 64);
...
function F = convert_index_to_vector(D, halfWinSize)
    F = zeros(size(D, 1), size(D, 2), 2);
    for j = 1:size(D, 1)
        for i = 1:size(D, 2)
            [y, x] = ind2sub([halfWinSize*2+1, halfWinSize*2+1], D(j, i) + 1);
            F(j, i, 1) = x - halfWinSize - 1;
            F(j, i, 2) = y - halfWinSize - 1;
        end
    end
